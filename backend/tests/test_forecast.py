...
def test_forecast_prediction_success(client, valid_forecast_payload):
    response = client.post("/predict/forecast", json=valid_forecast_payload)
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data['forecasted_sensor_values'], dict)
    assert len(data['forecasted_sensor_values']) > 0
    for sensor, values in data['forecasted_sensor_values'].items():
        assert len(values) == 2  # horizon is 2
        assert all(isinstance(v, float) and not (v != v) and v != float('inf') and v != float('-inf') for v in values)


def test_forecast_prediction_invalid_payload(client):
    invalid_payload = {
        "sensor_history": [
            {
                "time_in_cycles": 1,
    ...
    response = client.post("/predict/forecast", json=invalid_payload)
